...
func (m *memCache) LatestReport(ctx context.Context, req *pb.LatestReportRequest) (resp *pb.LatestReportResponse, err error) {
	...
	feedIDHex := bytesToFeedID(req.FeedId)
...
func (m *memCache) fetch(req *pb.LatestReportRequest, v *cacheVal) {
	...
	for {
		...
		if memcacheCtx.Err() != nil {
			...
			promFetchFailedCount.WithLabelValues(m.client.ServerURL(), bytesToFeedID(req.FeedId)).Inc()
			select {
			case <-m.chStop:
				return
...
func bytesToFeedID(b []byte) string {
	return common.BytesToHash(b).String()
}
