...
func TestValidation(t *testing.T) {
	...
	t.Run("ValidateValidFromTimestamp", func(t *testing.T) {
		...
		t.Run("fails when observationTimestamp is < validFromTimestamp", func(t *testing.T) {
			...
			require.EqualError(t, err, "observationTimestamp (Value: 111) must be >= validFromTimestamp (Value: 112)")
		})
	})
	t.Run("ValidateExpiresAt", func(t *testing.T) {
		...
		t.Run("fails when observationTimestamp > expiresAt", func(t *testing.T) {
			...
			require.EqualError(t, err, "expiresAt (Value: 111) must be ahead of observation timestamp (Value: 112)")
		})
	})
	t.Run("ValidateBetween", func(t *testing.T) {
		...
		require.EqualError(t, err, "test bar (Value: 346) is outside of allowable range (Min: 0, Max: 10)")
		err = ValidateBetween("test baz", bm, badMin, max)
		require.EqualError(t, err, "test baz (Value: 346) is outside of allowable range (Min: 9000, Max: 10000)")
	})
}
