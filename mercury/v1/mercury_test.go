...
type testReportCodec struct {
	...
	builtReportFields *v1.ReportFields
}
...
func newReportingPlugin(t *testing.T, codec *testReportCodec) *reportingPlugin {
	...
	return &reportingPlugin{
		f:               1,
		onchainConfig:   mercurytypes.OnchainConfig{Min: big.NewInt(0), Max: big.NewInt(1000)},
...
type mockDataSource struct{ obs v1.Observation }

func (m mockDataSource) Observe(context.Context, types.ReportTimestamp, bool) (v1.Observation, error) {
	return m.obs, nil
}
...
func Test_Plugin_Observation(t *testing.T) {
	...
	t.Run("with previous report", func(t *testing.T) {
		...
		t.Run("when all observations are successful", func(t *testing.T) {
			obs := v1.Observation{
				...
				CurrentBlockNum: mercurytypes.ObsResult[int64]{
					Val: rand.Int63(),
				},
				CurrentBlockHash: mercurytypes.ObsResult[[]byte]{
					Val: randBytes(32),
				},
				CurrentBlockTimestamp: mercurytypes.ObsResult[uint64]{
					Val: rand.Uint64(),
				},
				LatestBlocks: []v1.Block{
					v1.Block{Num: rand.Int63(), Hash: string(randBytes(32)), Ts: rand.Uint64()},
					v1.Block{Num: rand.Int63(), Hash: string(randBytes(32)), Ts: rand.Uint64()},
					v1.Block{Num: rand.Int63(), Hash: string(randBytes(32)), Ts: rand.Uint64()},
				},
			}
	...
	t.Run("without previous report, includes maxFinalizedBlockNumber observation", func(t *testing.T) {
		...
		obs := v1.Observation{
			...
			MaxFinalizedBlockNumber: mercurytypes.ObsResult[int64]{
...
func newAttributedObservation(t *testing.T, p *MercuryObservationProto) types.AttributedObservation {
	...
	return types.AttributedObservation{
		Observation: types.Observation(marshalledObs),
...
func Test_Plugin_parseAttributedObservation(t *testing.T) {
	t.Run("with all valid values, and > 0 LatestBlocks", func(t *testing.T) {
		...
		expectedLatestBlocks := make([]v1.Block, len(obs.LatestBlocks))
		for i, b := range obs.LatestBlocks {
			expectedLatestBlocks[i] = v1.NewBlock(b.Num, b.Hash, b.Ts)
		}
