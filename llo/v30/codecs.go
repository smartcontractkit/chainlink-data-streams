...
func GetOutcomeCodec(c protocol.OffchainConfig) OutcomeCodec {
...
func SelectBackfillCandidate(out *Outcome, backfillCID llotypes.ChannelID) (tsNanos uint64, rawTS uint64, opts protocol.HistoryBackfillOpts, uerr *UnreportableChannelError) {
	...
	if cd.ReportFormat != llotypes.ReportFormatHistoryBackfill {
		return 0, 0, protocol.HistoryBackfillOpts{}, &UnreportableChannelError{Inner: fmt.Errorf("internal error: not a history_backfill channel"), Reason: "not history_backfill", ChannelID: backfillCID}
	}
	...
	if !ok {
		return 0, 0, protocol.HistoryBackfillOpts{}, &UnreportableChannelError{Inner: fmt.Errorf("failed to backfill: target channel %d not in outcome", opts.TargetChannelID), Reason: "missing target channel", ChannelID: backfillCID}
	}
	res, err := protocol.TargetChannelTimeResolution(target)
	if err != nil {
		return 0, 0, protocol.HistoryBackfillOpts{}, &UnreportableChannelError{Inner: fmt.Errorf("failed to backfill: %w", err), Reason: "invalid target channel time resolution opts", ChannelID: backfillCID}
	}
	watermark, ok := out.ValidAfterNanoseconds[backfillCID]
	if !ok {
		return 0, 0, protocol.HistoryBackfillOpts{}, &UnreportableChannelError{Inner: nil, Reason: "IsReportable=false; no ValidAfterNanoseconds entry yet, this must be a new channel", ChannelID: backfillCID}
	}
	...
	if !found {
		return 0, 0, protocol.HistoryBackfillOpts{}, &UnreportableChannelError{Inner: nil, Reason: "backfill complete, no remaining timestamps", ChannelID: backfillCID}
	}
