...
func (protoOutcomeCodecV0) Encode(outcome Outcome) (ocr3types.Outcome, error) {
	...
	pbuf := &protocol.LLOOutcomeProtoV0{
...
func validAfterNanosecondsToProtoOutcomeSeconds(in map[llotypes.ChannelID]uint64) (out []*protocol.LLOChannelIDAndValidAfterSecondsProto, err error) {
	if len(in) > 0 {
		out = make([]*protocol.LLOChannelIDAndValidAfterSecondsProto, 0, len(in))
		for id, v := range in {
			seconds := v / 1e9
			if seconds > math.MaxUint32 {
				return nil, fmt.Errorf("valid after seconds too large: %d", seconds)
			}
			out = append(out, &protocol.LLOChannelIDAndValidAfterSecondsProto{
				ChannelID:         id,
...
func validAfterNanosecondsFromProtoOutcomeSeconds(in []*protocol.LLOChannelIDAndValidAfterSecondsProto) (out map[llotypes.ChannelID]uint64) {
	if len(in) > 0 {
