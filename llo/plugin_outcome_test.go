...
func Test_Outcome(t *testing.T) {
	...
	t.Run("if previousOutcome is retired, returns error", func(t *testing.T) {
		ctx := tests.Context(t)
		previousOutcome := Outcome{
		...
		_, err = p.Outcome(ctx, ocr3types.OutcomeContext{SeqNr: 2, PreviousOutcome: encodedPreviousOutcome}, types.Query{}, aos)
		assert.EqualError(t, err, "will not generate outcome for retired protocol instance")
	})
}
