...
func TestNewEnvAndRelease(t *testing.T) {
	...
	t.Run("multiple environments work independently", func(t *testing.T) {
		env1 := NewEnv(uint64(1750169759775700000))
		env2 := NewEnv(uint64(1750169759775700000))
...
func TestEnvAdd(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			id:    1,
			value: protocol.ToDecimal(decimal.NewFromFloat(123.45)),
			expectedEnv: map[string]any{
		...
		},
		{
			...
			id:   3,
			value: &protocol.TimestampedStreamValue{
				ObservedAtNanoseconds: 1234567890,
				StreamValue:           protocol.ToDecimal(decimal.NewFromFloat(123.45)),
			},
			expectedEnv: map[string]any{
