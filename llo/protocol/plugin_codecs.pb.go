...
//
// observationDueNanoseconds is the observation schedule: the timestamp at which
// each channel next becomes due for observation and aggregation. It is distinct
// from validAfterNanoseconds, which is a report boundary emitted in the report
// itself. The schedule advances at a fixed rate from its own previous value, so
// a cycle that runs late does not push every later cycle out with it. A channel
// with no entry is due; entries appear once a channel has reported.
//
// NOTE: must serialize deterministically, hence use of repeated tuple instead
// of maps. validAfterNanoseconds, reportableChannelIDs and
// observationDueNanoseconds MUST be sorted ascending by channelID;
// streamAggregates MUST be sorted ascending by (streamID, aggregator).
type LLOHotStateProto struct {
	...
	ObservationDueNanoseconds       []*LLOChannelIDAndObservationDueProto        `protobuf:"bytes,5,rep,name=observationDueNanoseconds,proto3" json:"observationDueNanoseconds,omitempty"`
	unknownFields                   protoimpl.UnknownFields
	sizeCache                       protoimpl.SizeCache
}
...
func (x *LLOHotStateProto) GetObservationDueNanoseconds() []*LLOChannelIDAndObservationDueProto {
	if x != nil {
		return x.ObservationDueNanoseconds
	}
	return nil
}

// LLOChannelIDAndObservationDueProto is one channel's observation schedule slot.
type LLOChannelIDAndObservationDueProto struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	ChannelID        uint32                 `protobuf:"varint,1,opt,name=channelID,proto3" json:"channelID,omitempty"`
	DueAtNanoseconds uint64                 `protobuf:"varint,2,opt,name=dueAtNanoseconds,proto3" json:"dueAtNanoseconds,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *LLOChannelIDAndObservationDueProto) Reset() {
	*x = LLOChannelIDAndObservationDueProto{}
	mi := &file_plugin_codecs_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LLOChannelIDAndObservationDueProto) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LLOChannelIDAndObservationDueProto) ProtoMessage() {}

func (x *LLOChannelIDAndObservationDueProto) ProtoReflect() protoreflect.Message {
	mi := &file_plugin_codecs_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use LLOChannelIDAndObservationDueProto.ProtoReflect.Descriptor instead.
func (*LLOChannelIDAndObservationDueProto) Descriptor() ([]byte, []int) {
	return file_plugin_codecs_proto_rawDescGZIP(), []int{18}
}

func (x *LLOChannelIDAndObservationDueProto) GetChannelID() uint32 {
	if x != nil {
		return x.ChannelID
	}
	return 0
...
func (x *LLOChannelIDAndObservationDueProto) GetDueAtNanoseconds() uint64 {
	if x != nil {
		return x.DueAtNanoseconds
	}
	return 0
}
...
var file_plugin_codecs_proto_goTypes = []any{
	...
	(*LLOChannelIDAndObservationDueProto)(nil),        // 19: v1.LLOChannelIDAndObservationDueProto
...
var file_plugin_codecs_proto_depIdxs = []int32{
	...
	19, // 18: v1.LLOHotStateProto.observationDueNanoseconds:type_name -> v1.LLOChannelIDAndObservationDueProto
	13, // 19: v1.LLOPrecursorProto.channelDefinitions:type_name -> v1.LLOChannelIDAndDefinitionProto
	15, // 20: v1.LLOPrecursorProto.validAfterNanoseconds:type_name -> v1.LLOChannelIDAndValidAfterNanosecondsProto
	16, // 21: v1.LLOPrecursorProto.streamAggregates:type_name -> v1.LLOStreamAggregate
