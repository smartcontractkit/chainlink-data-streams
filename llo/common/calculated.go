...
func ProcessCalculatedStreams(lggr logger.Logger, channelDefinitions llotypes.ChannelDefinitions, streamAggregates StreamAggregates, observationTimestampNanoseconds uint64, optsCache *OptsCache) {
	for cid, cd := range channelDefinitions {
		...
		copt, getErr := GetOpts[calculatedStreamOpts](optsCache, cid)
		if getErr != nil {
...
func evalExpression(o *calculatedStreamOpts, cid llotypes.ChannelID, env environment, streamAggregates StreamAggregates) error {
	for _, abi := range o.ABI {
		...
		streamAggregates[abi.ExpressionStreamID] = map[llotypes.Aggregator]StreamValue{
