...
func Test_protoOutcomeCodecV0(t *testing.T) {
	...
	t.Run("encode and decode with compression enabled", func(t *testing.T) {
		...
		codec, err := NewProtoOutcomeCodecV0(logger.Nop(), true)
		require.NoError(t, err)
	...
	t.Run("compressed and uncompressed data are different", func(t *testing.T) {
		...
		uncompressedCodec := protoOutcomeCodecV0{}
		compressedCodec, err := NewProtoOutcomeCodecV0(logger.Nop(), true)
		require.NoError(t, err)
...
func Fuzz_protoOutcomeCodecV0_Decode(f *testing.F) {
	...
	outcome := Outcome{}
	emptyPbuf, err := (protoOutcomeCodecV0{}).Encode(outcome)
	...
	valuesPbuf, err := (protoOutcomeCodecV0{}).Encode(outcome)
	require.NoError(f, err)
	...
	var codec OutcomeCodec = protoOutcomeCodecV0{}
...
func Test_protoOutcomeCodecV0_Properties(t *testing.T) {
	properties := gopter.NewProperties(nil)

	codec := protoOutcomeCodecV0{}
	...
	properties.Property("Encode/Decode", prop.ForAll(
		...
		gen.StrictStruct(reflect.TypeOf(&Outcome{}), map[string]gopter.Gen{
			...
			"ObservationTimestampNanoseconds": genUInt64NotLargerThanMaxInt64(),
			"ChannelDefinitions":              genChannelDefinitions(),
			"ValidAfterNanoseconds":           gen.MapOf(gen.UInt32(), genValidAfterNanosecondsTruncatedToUint32Seconds()),
...
func Test_protoOutcomeCodecV0_Properties_Compressed(t *testing.T) {
	properties := gopter.NewProperties(nil)
...
func genUInt64NotLargerThanMaxInt64() gopter.Gen {
	return gen.UInt64().Map(func(x uint64) uint64 {
		return x % (1 << 63)
	})
}

func genValidAfterNanosecondsTruncatedToUint32Seconds() gopter.Gen {
	return gen.UInt64().Map(func(x uint64) uint64 {
		if x/1e9 > math.MaxUint32 {
			return math.MaxUint32 * 1e9
		}
		return (x / 1e9) * 1e9 // we expect it to truncate nanoseconds
	})
}
