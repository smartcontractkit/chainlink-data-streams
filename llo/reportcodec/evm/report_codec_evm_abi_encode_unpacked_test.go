...
func genMaxReportRange() gopter.Gen {
	return gen.OneConstOf(protocol.Duration(0), protocol.Duration(60*time.Minute))
}
...
func TestReportCodecEVMABIEncodeUnpacked_Encode_properties(t *testing.T) {
		...
		runTest := func(sampleFeedID common.Hash, sampleObservationTimestampNanoseconds, sampleValidAfterNanoseconds uint64, sampleExpirationWindow uint32, priceMultiplier, marketDepthMultiplier *ubig.Big, sampleBaseUSDFee, sampleLinkBenchmarkPrice, sampleNativeBenchmarkPrice, sampleDexBasedAssetPrice, sampleBaseMarketDepth, sampleQuoteMarketDepth decimal.Decimal) bool {
			report := protocol.Report{
				...
				Values: []protocol.StreamValue{
					&protocol.Quote{Bid: decimal.NewFromFloat(6.1), Benchmark: sampleLinkBenchmarkPrice, Ask: decimal.NewFromFloat(8.2332)},  // Link price
					&protocol.Quote{Bid: decimal.NewFromFloat(9.4), Benchmark: sampleNativeBenchmarkPrice, Ask: decimal.NewFromFloat(11.33)}, // Native price
					protocol.ToDecimal(sampleDexBasedAssetPrice), // DEX-based asset price
					protocol.ToDecimal(sampleBaseMarketDepth),    // Base market depth
					protocol.ToDecimal(sampleQuoteMarketDepth),   // Quote market depth
				},
				Specimen: false,
			}
		...
		runTest := func(sampleFeedID common.Hash, sampleObservationTimestampNanoseconds, sampleValidAfterNanoseconds uint64, sampleExpirationWindow uint32, sampleBaseUSDFee, sampleLinkBenchmarkPrice, sampleNativeBenchmarkPrice, sampleMarketStatus decimal.Decimal) bool {
			report := protocol.Report{
				...
				Values: []protocol.StreamValue{
					&protocol.Quote{Bid: decimal.NewFromFloat(6.1), Benchmark: sampleLinkBenchmarkPrice, Ask: decimal.NewFromFloat(8.2332)},  // Link price
					&protocol.Quote{Bid: decimal.NewFromFloat(9.4), Benchmark: sampleNativeBenchmarkPrice, Ask: decimal.NewFromFloat(11.33)}, // Native price
					protocol.ToDecimal(sampleMarketStatus), // DEX-based asset price
				},
				Specimen: false,
			}
		...
		runTest := func(sampleFeedID common.Hash, sampleObservationTimestampNanoseconds, sampleValidAfterNanoseconds uint64, sampleExpirationWindow uint32, priceMultiplier, marketDepthMultiplier *ubig.Big, sampleBaseUSDFee, sampleLinkBenchmarkPrice, sampleNativeBenchmarkPrice, sampleBenchmarkPrice decimal.Decimal) bool {
			report := protocol.Report{
				...
				Values: []protocol.StreamValue{
					&protocol.Quote{Bid: decimal.NewFromFloat(6.1), Benchmark: sampleLinkBenchmarkPrice, Ask: decimal.NewFromFloat(8.2332)},  // Link price
					&protocol.Quote{Bid: decimal.NewFromFloat(9.4), Benchmark: sampleNativeBenchmarkPrice, Ask: decimal.NewFromFloat(11.33)}, // Native price
					protocol.ToDecimal(sampleBenchmarkPrice), // price
				},
				Specimen: false,
			}
	...
	t.Run("varying timestamp resolution schemas", func(t *testing.T) {
		runTest := func(sampleFeedID common.Hash, sampleObservationTimestampNanoseconds, sampleValidAfterNanoseconds uint64, sampleExpirationWindow uint32, priceMultiplier *ubig.Big, sampleBaseUSDFee, sampleLinkBenchmarkPrice, sampleNativeBenchmarkPrice, sampleBenchmarkPrice decimal.Decimal, sampleTimeResolution protocol.TimeResolution) bool {
			// Determine timestamp type based on resolution
			timestampType := "uint64"
			if sampleTimeResolution == protocol.ResolutionSeconds {
			...
			report := protocol.Report{
				...
				ObservationTimestampNanoseconds: sampleObservationTimestampNanoseconds,
				Values: []protocol.StreamValue{
					&protocol.Quote{Bid: decimal.NewFromFloat(6.1), Benchmark: sampleNativeBenchmarkPrice, Ask: decimal.NewFromFloat(8.2332)},
					&protocol.Quote{Bid: decimal.NewFromFloat(9.4), Benchmark: sampleLinkBenchmarkPrice, Ask: decimal.NewFromFloat(11.33)},
					protocol.ToDecimal(sampleBenchmarkPrice),
				},
				Specimen: false,
			}
			...
			checks := []bool{
				...
				assert.Equal(t, sampleBenchmarkPrice.Mul(decimal.NewFromBigInt(priceMultiplier.ToInt(), 0)).BigInt(), values[6].(*big.Int)),
			...
			expectedValidFrom := protocol.ConvertTimestamp(clampedValidAfterNanos, sampleTimeResolution) + 1
			expectedObservationTimestamp := protocol.ConvertTimestamp(sampleObservationTimestampNanoseconds, sampleTimeResolution)
			expectedExpiresAt := expectedObservationTimestamp + protocol.ScaleSeconds(sampleExpirationWindow, sampleTimeResolution)
		...
		runTest := func(sampleFeedID common.Hash, sampleObservationTimestampNanoseconds, sampleValidAfterNanoseconds uint64, sampleExpirationWindow uint32, sampleBaseUSDFee, sampleLinkBenchmarkPrice, sampleNativeBenchmarkPrice, sampleBinanceFundingRate, sampleBinanceFundingTime, sampleBinanceFundingIntervalHours, sampleDeribitFundingRate, sampleDeribitFundingTime, sampleDeribitFundingIntervalHours decimal.Decimal) bool {
			report := protocol.Report{
				...
				Values: []protocol.StreamValue{
					&protocol.Quote{Bid: decimal.NewFromFloat(6.1), Benchmark: sampleLinkBenchmarkPrice, Ask: decimal.NewFromFloat(8.2332)},  // Link price
					&protocol.Quote{Bid: decimal.NewFromFloat(9.4), Benchmark: sampleNativeBenchmarkPrice, Ask: decimal.NewFromFloat(11.33)}, // Native price
					protocol.ToDecimal(sampleBinanceFundingRate),          // Binance funding rate
					protocol.ToDecimal(sampleBinanceFundingTime),          // Binance funding time
					protocol.ToDecimal(sampleBinanceFundingIntervalHours), // Binance funding interval hours
					protocol.ToDecimal(sampleDeribitFundingRate),          // Deribit funding rate
					protocol.ToDecimal(sampleDeribitFundingTime),          // Deribit funding time
					protocol.ToDecimal(sampleDeribitFundingIntervalHours), // Deribit funding interval hours
				},
				Specimen: false,
			}
...
func TestReportCodecEVMABIEncodeUnpacked_Specimen(t *testing.T) {
...
func TestReportCodecEVMABIEncodeUnpacked_Encode(t *testing.T) {
	t.Run("ABI and values length mismatch error", func(t *testing.T) {
		report := protocol.Report{
			...
			Values: []protocol.StreamValue{
				&protocol.Quote{Bid: decimal.NewFromFloat(6.1), Benchmark: decimal.NewFromFloat(7.4), Ask: decimal.NewFromFloat(8.2332)},
				&protocol.Quote{Bid: decimal.NewFromFloat(9.4), Benchmark: decimal.NewFromFloat(10.0), Ask: decimal.NewFromFloat(11.33)},
				protocol.ToDecimal(decimal.NewFromFloat(100)),
				protocol.ToDecimal(decimal.NewFromFloat(101)),
				protocol.ToDecimal(decimal.NewFromFloat(102)),
			},
			Specimen: false,
		}
		...
		report.Values = []protocol.StreamValue{}
		_, err = codec.Encode(report, cd, cache)
...
func getMaxExpirationWindow() uint32 {
	// Use reasonable expiration window
	return 30 * 24 * 60 * 60
}

func genTimestampThatFitsUint32Seconds() gopter.Gen {
	// Leave headroom for max expirationWindow to prevent overflow when computing expiresAt
	maxSafeTimestamp := uint32(math.MaxUint32 - getMaxExpirationWindow())
	return gen.UInt32Range(0, maxSafeTimestamp).Map(func(i uint32) uint64 {
		return uint64(i) * 1e9
	})
}

func genTimeResolution() gopter.Gen {
	return gen.OneConstOf(protocol.ResolutionSeconds, protocol.ResolutionMilliseconds, protocol.ResolutionMicroseconds, protocol.ResolutionNanoseconds)
}

func genExpirationWindow() gopter.Gen {
	return gen.UInt32Range(1, getMaxExpirationWindow())
}
...
func TestReportCodecEVMABIEncodeUnpacked_EncodeOpts(t *testing.T) {
	t.Run("zero value is ResolutionSeconds", func(t *testing.T) {
		var defaultPrecision protocol.TimeResolution
		assert.Equal(t, protocol.ResolutionSeconds, defaultPrecision, "zero value must be ResolutionSeconds for backward compatibility")
		assert.Equal(t, protocol.TimeResolution(0), protocol.ResolutionSeconds, "ResolutionSeconds must be 0")
	})
	...
	t.Run("JSON opts without TimeResolution defaults to seconds", func(t *testing.T) {
		...
		assert.Equal(t, protocol.ResolutionSeconds, opts.TimeResolution)
	})
	...
	t.Run("JSON opts with TimeResolution uses correct value", func(t *testing.T) {
		testCases := []struct {
			...
			expectedPrecision protocol.TimeResolution
		}{
			{
				name:              "seconds",
				jsonPrecision:     "s",
				expectedPrecision: protocol.ResolutionSeconds,
			},
			{
				name:              "milliseconds",
				jsonPrecision:     "ms",
				expectedPrecision: protocol.ResolutionMilliseconds,
			},
			{
				name:              "microseconds",
				jsonPrecision:     "us",
				expectedPrecision: protocol.ResolutionMicroseconds,
			},
			{
				name:              "nanoseconds",
				jsonPrecision:     "ns",
				expectedPrecision: protocol.ResolutionNanoseconds,
			},
		}
