...
func Test_ABIEncoder_UnmarshalJSON(t *testing.T) {
	...
	t.Run("padded encoding", func(t *testing.T) {
		...
		res, err := enc.EncodePadded(protocol.ToDecimal(decimal.NewFromFloat32(123456.789123)))
		require.NoError(t, err)
		assert.Len(t, res, 32)
		...
		res, err = enc.EncodePadded(&protocol.TimestampedStreamValue{
			ObservedAtNanoseconds: 0x123456789,
			StreamValue:           protocol.ToDecimal(decimal.NewFromFloat32(123456.789123)),
		})
		require.NoError(t, err)
		assert.Len(t, res, 64)
	...
	t.Run("packed encoding", func(t *testing.T) {
		...
		res, err := enc.EncodePacked(protocol.ToDecimal(decimal.NewFromFloat32(123456.789123)))
		require.NoError(t, err)
		assert.Len(t, res, 24)
		...
		res, err = enc.EncodePacked(&protocol.TimestampedStreamValue{
			ObservedAtNanoseconds: 0x123456789,
			StreamValue:           protocol.ToDecimal(decimal.NewFromFloat32(123456.789123)),
		})
		require.NoError(t, err)
...
func Test_ABIEncoder_EncodePacked(t *testing.T) {
	t.Run("encodes decimals", func(t *testing.T) {
		...
		encoded, err := enc.EncodePadded(protocol.ToDecimal(decimal.NewFromFloat32(123456.789123)))
		require.NoError(t, err)
	...
	t.Run("errors on unsupported type (e.g. Quote)", func(t *testing.T) {
		...
		_, err := enc.EncodePacked(&protocol.Quote{})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "unhandled type; supported types are: *protocol.Decimal or *protocol.TimestampedStreamValue; got: *protocol.Quote")
	})
}
...
func Test_ABIEncoder_EncodePadded_EncodePacked(t *testing.T) {
	t.Run("encodes decimals", func(t *testing.T) {
		tcs := []struct {
			...
			{
				name:    "overflow int8",
				sv:      protocol.ToDecimal(decimal.NewFromFloat32(123456789.123456789)),
				abiType: "int8",
				errStr:  "value 123456790 out of range for type int8",
			},
			{
				name:           "successful int8",
				sv:             protocol.ToDecimal(decimal.NewFromFloat32(123.456)),
				abiType:        "int8",
			...
			},
			{
				...
				sv:             protocol.ToDecimal(decimal.NewFromFloat32(1.11)),
			...
			},
			{
				...
				sv:             protocol.ToDecimal(decimal.NewFromFloat32(123456.456)),
				abiType:        "uint32",
			...
			},
			{
				...
				sv:         protocol.ToDecimal(decimal.NewFromInt(math.MaxInt32)),
			...
			},
			{
				...
				sv:             protocol.ToDecimal(decimal.NewFromFloat32(123456.789123)),
				abiType:        "int192",
	...
	t.Run("encodes TimestampedStreamValue", func(t *testing.T) {
		tcs := []struct {
		...
		}{
			{
				...
				sv: &protocol.TimestampedStreamValue{
					ObservedAtNanoseconds: 0x123456789,
					StreamValue:           protocol.ToDecimal(decimal.NewFromFloat32(123456.789123)),
				},
				encoder: ABIEncoder{
			...
			},
			{
				...
				sv: &protocol.TimestampedStreamValue{
					ObservedAtNanoseconds: 0x123456789,
					StreamValue:           protocol.ToDecimal(decimal.NewFromFloat(float64(200))),
				},
				encoder: ABIEncoder{
					[]singleABIEncoder{{
						Type: "uint8",
			...
			},
			{
				...
				paddedErrStr: "unhandled type; supported nested types for *protocol.TimestampedStreamValue are: *protocol.Decimal; got: *protocol.Quote",
				packedErrStr: "failed to encode nested stream value; EncodePacked only currently supports StreamValue type of *protocol.Decimal, got: *protocol.Quote",
			},
			{
			...
			},
			{
				...
				paddedErrStr: "expected exactly two encoders for *protocol.TimestampedStreamValue; got: 1",
				packedErrStr: "expected exactly two encoders for *protocol.TimestampedStreamValue; got: 1",
			},
		}
