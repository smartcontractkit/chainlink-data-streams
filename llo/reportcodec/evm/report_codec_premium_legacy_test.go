...
import (
	...
	v3 "github.com/smartcontractkit/chainlink-data-streams/llo/reportcodec/evm/v3"
...
func FuzzReportCodecPremiumLegacy_Decode(f *testing.F) {
	...
	cache.Set(validReport.ChannelID, cd.Opts)

	validEncodedReport, err := codec.Encode(validReport, cd, cache)
...
func newValidPremiumLegacyReport() protocol.Report {
	return protocol.Report{
		...
		Values:                          []protocol.StreamValue{protocol.ToDecimal(decimal.NewFromInt(35)), protocol.ToDecimal(decimal.NewFromInt(36)), &protocol.Quote{Bid: decimal.NewFromInt(37), Benchmark: decimal.NewFromInt(38), Ask: decimal.NewFromInt(39)}},
		Specimen:                        false,
	}
}
...
func Test_ReportCodecPremiumLegacy(t *testing.T) {
	...
	t.Run("Encode errors if no values", func(t *testing.T) {
		cache := protocol.NewOptsCache()
		cache.Set(0, cd.Opts)
		_, err := rc.Encode(protocol.Report{}, cd, cache)
		require.Error(t, err)
	...
	t.Run("Encode constructs a report from observations", func(t *testing.T) {
		...
		encoded, err := rc.Encode(report, cd, cache)
		require.NoError(t, err)
	...
	t.Run("uses zero values if fees are missing", func(t *testing.T) {
		report := protocol.Report{
			Values: []protocol.StreamValue{nil, nil, &protocol.Quote{Bid: decimal.NewFromInt(37), Benchmark: decimal.NewFromInt(38), Ask: decimal.NewFromInt(39)}},
		}
		cache := protocol.NewOptsCache()
		cache.Set(report.ChannelID, cd.Opts)
...
var _ protocol.StreamValue = &UnhandledStreamValue{}
...
func (sv *UnhandledStreamValue) Type() protocol.LLOStreamValue_Type      { return 0 }

func Test_ExtractReportValues(t *testing.T) {
	t.Run("with wrong number of stream values", func(t *testing.T) {
		report := protocol.Report{Values: []protocol.StreamValue{protocol.ToDecimal(decimal.NewFromInt(35)), protocol.ToDecimal(decimal.NewFromInt(36))}}
	...
	t.Run("with (nil, nil, nil) values", func(t *testing.T) {
		report := protocol.Report{Values: []protocol.StreamValue{nil, nil, nil}}
		_, _, _, err := ExtractReportValues(report)

		require.EqualError(t, err, "ReportCodecPremiumLegacy expects third stream value to be of type *Quote; got: <nil>")
	})
	t.Run("with ((*protocol.Quote)(nil), nil, (*protocol.Quote)(nil)) values", func(t *testing.T) {
		report := protocol.Report{Values: []protocol.StreamValue{(*protocol.Quote)(nil), nil, (*protocol.Quote)(nil)}}
	...
	t.Run("with (*protocol.Decimal, *protocol.Decimal, *protocol.Decimal) values", func(t *testing.T) {
		report := protocol.Report{Values: []protocol.StreamValue{protocol.ToDecimal(decimal.NewFromInt(35)), protocol.ToDecimal(decimal.NewFromInt(36)), protocol.ToDecimal(decimal.NewFromInt(37))}}
		_, _, _, err := ExtractReportValues(report)

		require.EqualError(t, err, "ReportCodecPremiumLegacy expects third stream value to be of type *Quote; got: *protocol.Decimal")
	})
	t.Run("with ((*protocol.Quote)(nil), nil, *protocol.Quote) values", func(t *testing.T) {
		report := protocol.Report{Values: []protocol.StreamValue{(*protocol.Quote)(nil), nil, &protocol.Quote{Bid: decimal.NewFromInt(37), Benchmark: decimal.NewFromInt(38), Ask: decimal.NewFromInt(39)}}}
		nativePrice, linkPrice, quote, err := ExtractReportValues(report)

		require.NoError(t, err)
		assert.Equal(t, decimal.Zero, nativePrice)
		assert.Equal(t, decimal.Zero, linkPrice)
		assert.Equal(t, &protocol.Quote{Bid: decimal.NewFromInt(37), Benchmark: decimal.NewFromInt(38), Ask: decimal.NewFromInt(39)}, quote)
	})
	t.Run("with unrecognized types", func(t *testing.T) {
		report := protocol.Report{Values: []protocol.StreamValue{&UnhandledStreamValue{}, &UnhandledStreamValue{}, &UnhandledStreamValue{}}}
		_, _, _, err := ExtractReportValues(report)
		...
		report = protocol.Report{Values: []protocol.StreamValue{protocol.ToDecimal(decimal.NewFromInt(35)), &UnhandledStreamValue{}, &UnhandledStreamValue{}}}
		_, _, _, err = ExtractReportValues(report)
		...
		report = protocol.Report{Values: []protocol.StreamValue{protocol.ToDecimal(decimal.NewFromInt(35)), protocol.ToDecimal(decimal.NewFromInt(36)), &UnhandledStreamValue{}}}
		_, _, _, err = ExtractReportValues(report)
	...
	t.Run("with (*protocol.Decimal, *protocol.Decimal, *protocol.Quote) values", func(t *testing.T) {
		report := protocol.Report{Values: []protocol.StreamValue{protocol.ToDecimal(decimal.NewFromInt(35)), protocol.ToDecimal(decimal.NewFromInt(36)), &protocol.Quote{Bid: decimal.NewFromInt(37), Benchmark: decimal.NewFromInt(38), Ask: decimal.NewFromInt(39)}}}
		nativePrice, linkPrice, quote, err := ExtractReportValues(report)
	...
	t.Run("with (*protocol.Quote, *protocol.Quote, *protocol.Quote) values", func(t *testing.T) {
		report := protocol.Report{Values: []protocol.StreamValue{&protocol.Quote{Bid: decimal.NewFromInt(35), Benchmark: decimal.NewFromInt(36), Ask: decimal.NewFromInt(37)}, &protocol.Quote{Bid: decimal.NewFromInt(38), Benchmark: decimal.NewFromInt(39), Ask: decimal.NewFromInt(40)}, &protocol.Quote{Bid: decimal.NewFromInt(41), Benchmark: decimal.NewFromInt(42), Ask: decimal.NewFromInt(43)}}}
		nativePrice, linkPrice, quote, err := ExtractReportValues(report)

		require.NoError(t, err)
		assert.Equal(t, decimal.NewFromInt(36), nativePrice)
		assert.Equal(t, decimal.NewFromInt(39), linkPrice)
		assert.Equal(t, &protocol.Quote{Bid: decimal.NewFromInt(41), Benchmark: decimal.NewFromInt(42), Ask: decimal.NewFromInt(43)}, quote)
	})
	t.Run("with (nil, nil, *protocol.Quote) values", func(t *testing.T) {
		report := protocol.Report{Values: []protocol.StreamValue{nil, nil, &protocol.Quote{Bid: decimal.NewFromInt(37), Benchmark: decimal.NewFromInt(38), Ask: decimal.NewFromInt(39)}}}
		nativePrice, linkPrice, quote, err := ExtractReportValues(report)
