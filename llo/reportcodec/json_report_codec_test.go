...
func Test_JSONCodec_Properties(t *testing.T) {
	...
	properties.Property("Encode/Decode", prop.ForAll(
		...
		gen.StrictStruct(reflect.TypeOf(&protocol.Report{}), map[string]gopter.Gen{
...
func equalReports(r, r2 protocol.Report) bool {
...
func Test_JSONCodec(t *testing.T) {
	t.Run("Encode=>Decode", func(t *testing.T) {
		r := protocol.Report{
			...
			Values:                          []protocol.StreamValue{protocol.ToDecimal(decimal.NewFromInt(1)), protocol.ToDecimal(decimal.NewFromInt(2)), &protocol.Quote{Bid: decimal.NewFromFloat(3.13), Benchmark: decimal.NewFromFloat(4.4), Ask: decimal.NewFromFloat(5.12)}},
			Specimen:                        true,
		}
	...
	t.Run("UnpackDecode unpacks and decodes report", func(t *testing.T) {
		...
		assert.Equal(t, protocol.Report{
			ConfigDigest:                    types.ConfigDigest([32]byte{1, 2, 3}),
			SeqNr:                           43,
			ChannelID:                       llotypes.ChannelID(46),
			ValidAfterNanoseconds:           44,
