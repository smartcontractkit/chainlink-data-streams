...
var _ protocol.ReportCodec = JSONReportCodec{}

// JSONReportCodec is a chain-agnostic reference implementation

type JSONReportCodec struct{}

func (cdc JSONReportCodec) Encode(r protocol.Report, _ llotypes.ChannelDefinition, _ *protocol.OptsCache) ([]byte, error) {
	type encode struct {
		...
		Values                          []protocol.TypedTextStreamValue
		Specimen                        bool
	}
	values := make([]protocol.TypedTextStreamValue, len(r.Values))
	for i, sv := range r.Values {
		t, err := protocol.NewTypedTextStreamValue(sv)
		if err != nil {
			return nil, fmt.Errorf("failed to encode protocol.StreamValue: %w", err)
		}
		values[i] = t
	}
	e := encode{
...
func (cdc JSONReportCodec) Decode(b []byte) (r protocol.Report, err error) {
	...
	values := make([]protocol.StreamValue, len(d.Values))
	for i := range d.Values {
		values[i], err = protocol.UnmarshalTypedTextStreamValue(&d.Values[i])
		if err != nil {
			return r, fmt.Errorf("failed to decode protocol.StreamValue: %w", err)
		}
	}
